...
class FeatureCost():
    ...
    def _batchify_computation(self, X, side='x', slices=20):
        if side == 'x':
            out = torch.cat([self.src_emb(b).to(self.device) for b in torch.chunk(X, slices, dim=0)])
            #print("Device: ", device)
            #print("X,Device: ", X.device)
        else:
            out = torch.cat([self.tgt_emb(b).to(self.device) for b in torch.chunk(X, slices, dim=0)])
            #print("Device: ", device)
            #print("X,Device: ", X.device)
